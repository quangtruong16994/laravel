	...
	function LoadDatatables(){
		$.getScript('js/jquery.dataTables.js', function(){
			$.getScript('ZeroClipboard.js', function(){
				$.getScript('js/TableTools.js', function(){
					$.getScript('js/dataTables.bootstrap.js', callback);
				});
			});
		});
	}
...
function fadeIn(elem, ms) {
	...
	if (ms) {
		...
		var timer = setInterval(function() {
			...
			elem.style.filter = "alpha(opacity=" + opacity * 80 + ")";
		}, 50);
	} else {
...
function validateLogin(fn) {
	...
	if (pass.trim() == "") {
		fadeIn('message', 1000);
		document.getElementById('message').innerHTML = 'Yêu cầu nhập mật khẩu.';
	...
	if (name.trim() == "") {
		fadeIn('message', 1000);
		document.getElementById('message').innerHTML = 'Yêu cầu nhập tên đăng nhập.';
	...
	if (name.trim() == "" && pass.trim() == "") {
		fadeIn('message', 1000);
		document.getElementById('message').innerHTML = 'Yêu cầu nhập đầy đủ tên đăng nhập và mật khẩu.';
		fn.txtUsername.focus();
		validName = false;
		validPass = false;
	}
	/*else {
		if (pass.length >= 8) {
			fadeIn('message', 1000);
			document.getElementById('message').innerHTML = 'Độ dài mật khẩu phải nhỏ hơn 8 ký tự.';
			fn.txtUserPass.focus();
			fn.txtUserPass.select();
			return false;
		}
...
//		document.getElementById("login-form").setAttribute("action", "UserLogin");		
//	}
...
function validateAccount(fn){
	
	document.getElementById('mess1').style.opacity = 0;
	document.getElementById('mess2').style.opacity = 0;
	document.getElementById('mess3').style.opacity = 0;
	document.getElementById('mess4').style.opacity = 0;
	document.getElementById('mess5').style.opacity = 0;
	
	var userName = fn.txtUserName.value;
	var userPass = fn.txtUserPass.value;
	var userRePass = fn.txtUserRePass.value;
	var userEmail = fn.txtUserEmail.value;
	var UserType = document.getElementById('UserType');
	var userType = UserType.options[UserType.selectedIndex].value;
	
	var patternUser = /^[a-zA-Z0-9]{5,15}$/;
	var patternEmail = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
	var patternPass = /^[A-Za-z0-9!@#$%^&*()_]{6,20}$/;
	
	var validName = true;
	var validPass = true;
	var validRePass = true;
	var validEmail = true;
	var validUserType = true;
	
	if(userName.trim() == ""){
		fadeIn('mess1', 1000);
		document.getElementById('mess1').innerHTML = 'Yêu cầu nhập tên đăng nhập cho tài khoản.';
		validName = false;
	} else {
		if(!userName.match(patternUser)) {
			fadeIn('mess1', 1000);
			document.getElementById('mess1').innerHTML = 'Tên đăng nhập phải từ 5 đến 15 ký tự và không chứa ký tự đặc biệt.';
			validName = false;
		}
	}
	...
	if(userPass.trim() == ""){
		fadeIn('mess2', 1000);
		document.getElementById('mess2').innerHTML = 'Yêu cầu nhập mật khẩu cho tài khoản.';
		validPass = false;
	}
	else{
		if(!userPass.match(patternPass)){
			fadeIn('mess2', 1000);
			document.getElementById('mess2').innerHTML = 'Độ dài mật khẩu phải từ 6 đến 20 ký tự.';
			validPass = false;
		}
		else{
			if(userRePass.trim() == ""){
				fadeIn('mess3', 1000);
				document.getElementById('mess3').innerHTML = 'Yêu cầu nhập lại mật khẩu.';
				validRePass = false;
			} else{
				if(userPass.trim() != userRePass.trim()) {
					fadeIn('mess3', 1000);
					document.getElementById('mess3').innerHTML = 'Mật khẩu nhập lại không trùng. Yêu cầu nhập lại.';
					validRePass = false;
				}
			}
		}
	...
	if(userEmail.trim() == ""){
		fadeIn('mess4', 1000);
		document.getElementById('mess4').innerHTML = 'Yêu cầu nhập tên hộp thư.';
		validEmail = false;
	} else {
		if(!userEmail.match(patternEmail)){
			fadeIn('mess4', 1000);
			document.getElementById('mess4').innerHTML = 'Cấu trúc hộp thư không đúng.';
			validEmail = false;
		}
	}
	...
	if(userType.trim() == "0"){
		fadeIn('mess5', 1000);
		document.getElementById('mess5').innerHTML = 'Yêu cầu chọn loại tài khoản.';
		validUserType = false;
	}
	
	if(!validUserType) {
		fn.slcUserType.focus();
	}
	if(!validEmail) {
		fn.txtUserEmail.focus();
	}
	if(!validRePass) {
		fn.txtUserRePass.focus();
	}
	if(!validPass) {
		fn.txtUserPass.focus();
	}
	...
	return validName && validPass && validRePass && validEmail && validUserType;
}
...
$(document).ready(function(){
    $(".update").click(function(){
    	...
    	var accId = par.children("td:nth-child(2)");
    	var accName = par.children("td:nth-child(3)");
    	var accPass = par.children("td:nth-child(4)"); 
    	var accEmail = par.children("td:nth-child(5)"); 
    	var accType = par.children("td:nth-child(6)"); 
    	var accState = par.children("td:nth-child(7)");
    	
    	var s1="", s2="", s3="", s4="", s5="";
    	var acctype = accType.text();
    	if(acctype=="Nhân viên"){
    		s1="selected";
    	}
    	if(acctype=="Trưởng phòng"){
    		s2="selected";
    	}
    	if(acctype=="Giám đốc"){
    		s3="selected";
    	...
    	if(acctype=="Quản trị cấp cao"){
    		s5="selected";
    	}
    	
    	document.getElementById('accId').value = accId.text();
    	document.getElementById('accName').innerHTML = accName.text();
    	document.getElementById('accPass').value = accPass.text();
    	document.getElementById('accEmail').value = accEmail.text();
    });
});
